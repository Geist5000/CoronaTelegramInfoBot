...
class StateSource(DataSource):
    ...
    def getRawData(self):
        if (self.lastUpdate == None) or ((datetime.now() - self.lastUpdate).days >= 1):
            ...
            vaccinationUrl = "https://www.rki.de/DE/Content/InfAZ/N/Neuartiges_Coronavirus/Daten/Impfquotenmonitoring.xlsx?__blob=publicationFile"
            df = pd.read_excel(BytesIO(requests.get(vaccinationUrl).content), sheet_name=1)
            self.vaccinationData = df.query(f"Bundesland == '{self.getRegion().name}'").iloc[0]
    ...
    def getVaccCum(self):
        return int(self.vaccinationData["Impfungen kumulativ"])
    def getVaccDiff(self):
        return int(self.vaccinationData["Differenz zum Vortag"])
    def getVacc1000(self):
        return self.vaccinationData["Impfungen pro 1.000 Einwohner"]
